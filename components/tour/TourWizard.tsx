// components/tour/TourWizard.tsx
"use client";
...
export default function TourWizard({ tourId: initialTourId }: { tourId?: string }) {
  const isEditMode = !!initialTourId;

  // In edit mode, start at step 1 but with tourId already set
  // so Step1Basic loads existing data immediately
  const [step, setStep] = useState(1);
  const [furthestStep, setFurthestStep] = useState(
    isEditMode ? STEPS.length : 1  // In edit mode, all steps are unlocked
  );
  const [tourId, setTourId] = useState<string | null>(initialTourId ?? null);
